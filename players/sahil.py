...
def my_neighbours(i,j):
    dim =11
    neighbours = get_neighbours(dim,(i,j))
    if i-2>=0 and j-1>=0:
        neighbours.append((i-2,j-1))
    if i-1>=0 and j-2>=0:
        neighbours.append((i-1,j-2))
    ...
    def __init__(self,state,player):
        ...
        if opponent == "TA":
            ...
            if total_moves<=6:
                ...
                self.max_time = min(16,2*current_remaining_time/len((self.all_moves))*1.5)
            elif total_moves <= 15:
                self.max_time = min(16,2*current_remaining_time/len((self.all_moves))*2)
            elif total_moves <= 25:
                self.max_time = min(16,2*current_remaining_time/len((self.all_moves))*2)
            else:
                self.max_time = 2*current_remaining_time/(len(self.all_moves))
